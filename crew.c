...
Crew_data deleteIdSpaceship(Crew_data data, int index, int index_spaceship){
    if(data.crew[index].id_spaceship.n_spaceships < 1){
        ...
        Crew* aux = realloc(data.crew[index].id_spaceship.id_spaceship, (data.crew[index].id_spaceship.n_spaceships - 1)*sizeof(Id_Spaceship));
...
Crew_data fireCrew(Crew_data data){
    ...
    viewCrew(data);
    printf("Introduce el ID del tripulante que quieres eliminar: ");
    first = 0;
    while(id_crew == -1){
        if(first == 0){
            printf("\n(ERROR) El ID introducido es de un formato incorrecto, porfavor introduce un ID: ");
            first = 1;
        }
        askForString(input, "(ERROR) El ID introducido es de un formato incorrecto, porfavor introduce un ID: ");
