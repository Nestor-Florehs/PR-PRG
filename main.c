...
void executeMainMenu(int option, DataUsers* dataUsers, DataSpaceship* ds, Crew_data* dc, DataSpaceship stock, VoyageData* voyage_data) {
    ...
    switch(option) {
        case 1:
            ...
            if (user.rol == 'A') {
                do {
                    ...
                    if(projectOption != -1) {
                        executeAdminMenu(projectOption, dataUsers, ds, dc, stock, user.email,voyage_data);
                    }else {
        ...
        if(user.rol == 'C') {
            String optionUser;
            do {
                showClientMenu(dataUsers, voyage_data);
                askForString(optionUser, "Choose an option: ");
                projectOption = checkInt(optionUser);
                if(projectOption != -1) {
                    executeClientMenu(projectOption, dataUsers, voyage_data, stock);
                }else {
                    printf("Please enter a valid option!");
                }
...
int main() {
    ...
    do {
        ...
        scanf("%d", &option);
        scanf("%c", &aux);
        executeMainMenu(option, &dataUsers, &dataSpaceship, &dataCrew, dataSpaceship,&dataVoyage);
    }while(option != 3);
