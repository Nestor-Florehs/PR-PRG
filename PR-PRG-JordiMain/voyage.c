...
VoyageData rateVoyage(VoyageData dv, DataUsers du, int idx_user){
...
int checkEmail(String email, DataUsers du) {
    for (int i = 0; i < du.n_users; i++) {
        if (strcmp(email, du.user[i].email) == 0) {
            return 1;
        }
    }
    return 0;
...
int getIdFromEmail(DataUsers du, String email) {
    for (int i = 0; i < du.n_users; i++) {
        if (strcmp(du.user[i].email, email) == 0) {
            return du.user[i].id;
        }
    }
    return 0;
...
VoyageData giveVoyage(VoyageData dv, DataUsers du, int idx_user) {
    int id, found = 0, i, j, rating;
    LongString comment;
    String aux;
    ...
    if(found == 0){
        ...
        if(found == 0){
            ...
            String mail;
            int id;
            askForString(mail, "Enter the email to who you want to give the voyage: ");
            if (checkEmail(mail, du)) {
                id = getIdFromEmail(du, mail);
                dv.voyages[i].tickets[j].id_passenger = id;
                printf("Your ticket voyage has been gifted to %s\n", mail);
            }else {
